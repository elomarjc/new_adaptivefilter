...
mu_LMS = 0.1; 
w_LMS = zeros(M_LMS, 1); 
padded_signal = [zeros(M_LMS-1, 1); primary]; 
output_LMS = zeros(minLength, 1);

for n = 1:minLength
    u_vect = padded_signal(n:n+M_LMS-1); 
...
M = 1;
for n = 1:minLength
    u_vect = padded_signal(n:n+M_NLMS-1);
...
%% RLS Filter
M_RLS = 12;
lambda = 1 - 1 / (0.1 * M_RLS); % Forgetting factor  1 - 1 / (0.1 * M) =  0.1666 ; 
delta = 0.01; 
P = 1 / delta * eye(M_RLS); 
w_RLS = zeros(M_RLS, 1);
...
for n = 1:minLength
    u_vect = padded_signal_rls(n:n+M_RLS-1); 
    PI = P * u_vect; 
...
% Save figure
tightfig();
saveas(gcf, 'Frequency Response of Final Weights - Sinus Wave.pdf');
...
fprintf('Filter order (LMS, NLMS, RLS): %f, %f, %f \n', M_LMS, M_NLMS, M_RLS);
