...
for n = 1:N
    ...
    e(n) = noise(n) - output_LMS(n); % Error signal
    w_LMS = w_LMS + mu_LMS * e(n) * u_vect; % Update weights
end 
...
for n = 1:N
    ...
    e(n) = noise(n) - output_NLMS(n); % Error signal
    mu_adapt = mu_NLMS / (Eps + norm(u_vect)^2); % Adaptive step size
    w_NLMS = w_NLMS + mu_adapt * e(n) * u_vect; % Update weights
end

filtered_signal_NLMS = primary - output_NLMS; 

%% RLS Filter
lambda = 0.9010; % Forgetting factor (adjusted from first code)
...
for n = 1:N
    ...
    e(n) = noise(n) - output_RLS(n); % Error signal
    PI = P * u_vect; % Intermediate calculation
    gain_k = PI / (lambda + u_vect' * PI); % Gain
    w_RLS = w_RLS + gain_k * e(n); % Update weights
...
audiowrite(fullfile(newFolder, 'Filtered_LMS_2.wav'), output_LMS_normalized, fs);
audiowrite(fullfile(newFolder, 'Filtered_NLMS_2.wav'), output_NLMS_normalized, fs);
audiowrite(fullfile(newFolder, 'Filtered_RLS_2.wav'), output_RLS_normalized, fs);
...
tightfig();
saveas(gcf, 'sinus_wave_filter_2.pdf');
