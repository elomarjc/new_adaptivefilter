...
initial_SNR = 10 * log10(sum(x.^2) / sum((x - d).^2));  % The same as 10 * log10(sum(x.^2) / sum(d.^2))
...
parfor idx = 1:totalIterations
    ...
    if idx <= totalMu
        ...
        if all(isfinite(y))
            snr_val = 10 * log10(sum(x.^2) / sum((x - (u-y)).^2));
...
% Save figure
tightfig();
saveas(gcf, fullfile(foldername, 'Output SNR vs Filter Length - Synthetic Heart.pdf'));
...
% Save the figure
tightfig();
saveas(gcf, fullfile(foldername, 'Mel_Spectrogram - Synthetic Heart.pdf'));
...
% Save figure
tightfig();
saveas(gcf, fullfile(foldername, 'Error signals - Synthetic Heart.pdf'));
...
% Save figure
tightfig();
saveas(gcf, fullfile(foldername, 'Frequency Response of Final Weights - Synthetic Heart.pdf'));
