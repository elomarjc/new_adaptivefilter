...
%% Parameters
mu_values_LMS = 1e-3 * (2.^(0:9));;   % Separate step size range for LMS
mu_values_NLMS = 1e-3 * (2.^(0:9));    % Separate step size range for NLMS
filterOrders = 1:15;
...
filename_base = sprintf('Order_%d-%d_StepSize_%s-%s-%s', ...
...
output_LMS_normalized = (primary - y_LMS) / max(abs(primary - y_RLS));
output_NLMS_normalized = (primary - y_NLMS) / max(abs(primary - y_RLS));
output_RLS_normalized = (primary - y_RLS) / max(abs(primary - y_RLS));
...
% Save figure
tightfig();
saveas(gcf, fullfile(foldername, 'Output SNR vs Filter Order.pdf'));
...
% Save figure
tightfig();
saveas(gcf, fullfile(foldername, 'Best parameter vs filter order.pdf'));
...
% Save figure
tightfig();
saveas(gcf, fullfile(foldername, 'Maximum SNR Achieved by Each Algorithm.pdf'));
...
% Save figure
tightfig();
saveas(gcf,  fullfile(foldername, 'Mel_Spectrogram_sinus_wave.pdf'));
...
% Save figure
tightfig();
saveas(gcf, fullfile(foldername, 'sinus_wave_error_signals.pdf'));
...
% Save figure
tightfig();
saveas(gcf, fullfile(foldername, 'Frequency Response of Final Weights.pdf'));
