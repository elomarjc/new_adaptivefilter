...
d = 0.5 * randn(size(t))';   % Desired signal      Secondary
u = x + d;                   % Observed signal     Primary
...
bestParamsLMS = zeros(length(filterOrders), 3); % [Filter Length, Best SNR (LMS), Optimal mu (LMS)]
bestParamsNLMS = zeros(length(filterOrders), 3); % [Filter Length, Best SNR (NLMS), Optimal mu (NLMS)]
bestParamsRLS = zeros(length(filterOrders), 3); % [Filter Length, Best SNR (RLS), Optimal lambda (RLS)]
